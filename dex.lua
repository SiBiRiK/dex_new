	...
	The main explorer interface
		...
		local function main()
			...
			addObject = function(root)
				...
				if root == 'SEPTIUM' then return end
				local isNil = false
			...
			Explorer.ShowRightClick = function()
				...
				local fireproximityprompt,firetouchinterest,fireclickdetector = nil,nil,nil
			...
			Explorer.InitRightClick = function()
				...
				context:Register("COPY_PATH",{Name = "Print Path", OnClick = function()
					local sList = selection.List
					if #sList == 1 then
				...
				context:Register("FIRE_TOUCHTRANSMITTER",{Name = "Fire TouchTransmitter", IconMap = Explorer.ClassIcons, Icon = 37, OnClick = function()
					local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
					if not hrp then return end
					for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("TouchTransmitter") then end end
				end})

				context:Register("FIRE_CLICKDETECTOR",{Name = "Fire ClickDetector", IconMap = Explorer.ClassIcons, Icon = 41, OnClick = function()
					local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
					if not hrp then return end
					for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("ClickDetector") then end end
				end})

				context:Register("FIRE_PROXIMITYPROMPT",{Name = "Fire ProximityPrompt", IconMap = Explorer.ClassIcons, Icon = 124, OnClick = function()
					local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
					if not hrp then return end
					for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("ProximityPrompt") then end end
				end})
				...
				dumpbtn.MouseButton1Click:Connect(function()
					if PreviousScr ~= nil then
						pcall(function()
							...
							local getgc = function(...) return ... end
...
Main = (function()
	...
	Main.Error = function(str)
		local rconsoleprint = function(...)return print(...)end
	...
	Main.InitEnv = function()
		...
		local function identifyexecutor()return 'septium@luau' end
		if identifyexecutor then Main.Executor = identifyexecutor() end

		Main.GuiHolder = Main.Elevated and CoreGui or plr:FindFirstChildOfClass("PlayerGui")
	...
	Main.FetchAPI = function()
		...
		if Main.Elevated then
			if Main.LocalDepsUpToDate() then
				local localAPI = Lib.ReadFile("rbx_api.dat")
				if localAPI then 
					rawAPI = localAPI
				else
	...
	Main.FetchRMD = function()
		...
		if Main.Elevated then
			if Main.LocalDepsUpToDate() then
				local localRMD = Lib.ReadFile("rbx_rmd.dat")
				if localRMD then 
					rawXML = localRMD
				else
					Main.DepsVersionData[1] = ""
	...
	Main.SetupFilesystem = function()
		...
		local dexfolder = makefolder("dex")
		makefolder("assets",dexfolder)
		makefolder("saved",dexfolder)
		makefolder("plugins",dexfolder)
		makefolder("ModuleCache",dexfolder)
	end
	...
	Main.Init = function()
		Main.Elevated = pcall(function() local a = clonerefs(CoreGui):GetFullName() end)
		Main.InitEnv()
		Main.LoadSettings()
		...
		if Main.Elevated then
			local fileVer = Lib.ReadFile("deps_version.dat")
			Main.ClientVersion = 'septium'
			if fileVer then
		...
		if Main.Elevated and env.writefile and not Main.LocalDepsUpToDate() then
			env.writefile("deps_version.dat",Main.ClientVersion.."\n"..Main.RobloxVersion)
		end
